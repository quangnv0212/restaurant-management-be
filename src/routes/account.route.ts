...
import {
  ...
  UpdateMeBody,
  UpdateMeBodyType,
  AccountListWithPaginationQuery,
  AccountListWithPaginationQueryType,
  AccountListWithPaginationRes,
  AccountListWithPaginationResType
...
export default async function accountRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  ...
  fastify.get<{
    Reply: AccountListWithPaginationResType
    Querystring: AccountListWithPaginationQueryType
  }>(
    '/',
    {
      schema: {
        response: {
          200: AccountListWithPaginationRes
        },
        querystring: AccountListWithPaginationQuery
      },
      preValidation: fastify.auth([requireOwnerHook])
    },
    async (request, reply) => {
      const { page, limit, sortBy, sortOrder, search } = request.query
      const data = await getAccountListWithPagination(page, limit, sortBy, sortOrder, search)
      reply.send({
        data: {
          items: data.items as AccountListWithPaginationResType['data']['items'],
