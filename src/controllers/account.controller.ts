...
export const initOwnerAccount = async () => {
  ...
  if (accountCount === 0) {
    ...
    console.log(
      chalk.bgCyan(
        `Create owner account successfully: ${envConfig.INITIAL_EMAIL_OWNER}|${envConfig.INITIAL_PASSWORD_OWNER}`
      )
    )
  }
}
...
export const updateEmployeeAccount = async (accountId: number, body: UpdateEmployeeAccountBodyType) => {
  try {
    ...
    if (!oldAccount) {
      throw new EntityError([{ field: 'email', message: 'Account you are updating no longer exists!' }])
    }
...
export const getAccountListWithPagination = async (
  page: number,
  limit: number,
  sortBy: string,
  sortOrder: string,
