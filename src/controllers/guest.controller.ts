...
export const guestLoginController = async (body: GuestLoginBodyType) => {
  ...
  if (table.status === TableStatus.Hidden) {
    throw new Error('Table is hidden, please choose another table to login')
  }

  if (table.status === TableStatus.Reserved) {
    throw new Error('Table is reserved, please contact staff to get support')
  }
...
export const guestCreateOrdersController = async (guestId: number, body: GuestCreateOrdersBodyType) => {
  const result = await prisma.$transaction(async (tx) => {
    ...
    if (guest.tableNumber === null) {
      throw new Error('Table of you is deleted, please logout and login again to a new table')
    }
    ...
    if (table.status === TableStatus.Hidden) {
      throw new Error(`Table ${table.number} is hidden, please logout and choose another table`)
    }
    if (table.status === TableStatus.Reserved) {
      throw new Error(`Table ${table.number} is reserved, please logout and choose another table`)
    }
